...
| Notifications | ✅ Admin inbox (search/filter/mark read, type & read tabs), broadcast to all/administrator/resident accounts, emergency alerts (severity, purok targeting) with per-user notifications |
| Settings | ✅ Barangay/general settings (saved to `config/app.php`), user management (list/search/filter, create accounts with role & status) |
...
### 26. Notifications module — "Under Construction" stub replaced
The `/admin/notifications` page only rendered the generic "Under Construction" placeholder. Implemented the module on the `notifications`, `emergency_alerts`, and `notification_templates` tables: an **admin inbox** (the logged-in official's own notifications) with statistics (total/unread/read/emergency), search, type and read-status tabs, unread highlighting, pagination, and mark-as-read per item; a **broadcast form** that inserts one `notifications` row per active user account for the chosen audience (all users / barangay officials / resident accounts), optionally picking the message from a `notification_templates` preset; and an **emergency alert** page (severity low/medium/high/critical, optional target puroks/sitios stored as JSON, and a recent-alerts ledger) where sending an alert inserts the `emergency_alerts` record and an emergency `notifications` row for every active user in one transaction. Three things surfaced during verification: the table needs an explicit `n` alias *and* a `WHERE` keyword when building the dynamic where clause (the query read `FROM notifications n n.deleted_at…` without `WHERE`, exactly the class of issue logged in #20), the emergency routes were nested under `/admin/notifications/emergency/send` so RBAC extracts module `notifications` (the original `/admin/emergency/send` mapped to module `emergency`, for which no permissions exist, so only the captain — exempt from the module check — could open the page), and `emergency_alerts.target_puroks` is a JSON column, so the selected purok IDs are `json_encode()`d on insert (the same constraint noted in #18 for `tanod_schedules.assigned_members`).
